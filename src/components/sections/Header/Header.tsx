   ...
   return (
      <header className={`${className || ''}`}>
         <div className={`container`}>
            ...
            >
               <nav
                  className={`flex w-full items-center ${path === '/login' && 'justify-between'}`}
               >
                  <Logo
                     className={'mr-8'}
                     color={path === '/login' ? 'white' : 'black'}
                  />
                  <HeaderMenu
                     className={`mr-10 ${path == '/login' && 'hidden'}`}
                  />
                  <HeaderInput
                     className={`mr-8 ${path == '/login' && 'hidden'}`}
                  />

                  {path === '/login' && <HeaderLanguages />}
                  <div
                     className={`flex w-full items-center gap-5 ${path == '/login' && 'hidden'}`}
                  >
                     <HeaderLanguages />
