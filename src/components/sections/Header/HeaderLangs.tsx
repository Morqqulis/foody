...
const iconStyles =
   'lg:min-w-10 lg:min-h-10 min-w-8 min-h-8 rounded-full group-hover/langItem:scale-110'

interface IHeaderLangs {
   className?: string
}

const HeaderLanguages: React.FC<IHeaderLangs> = ({
   className
}: IHeaderLangs): JSX.Element => {
   ...
   return (
      <DropdownMenu>
         <DropdownMenuTrigger className={`group/langItem`}>
            {currentLocale && (
               <Image
                  ...
                  src={`/Header/langs/${currentLocale}.svg`}
                  width={40}
                  height={40}
                  alt={`${currentLocale}`}
               />
         ...
         <DropdownMenuContent
            className={`flex w-fit min-w-fit flex-col items-center justify-center`}
         >
