      ...
      <section className={`py-20`}>
         <div className="container">
            <div className={`text-center`}>
               <Title
                  className={`text-[40px] font-black leading-[70px]`}
                  tag={'h2'}
                  text={t('title')}
               />
               <p
                  className={`mx-auto mb-14 max-w-[780px] text-[22px] leading-[30px]`}
               >
                  {t('text')}
               </p>
            </div>
            <div className="grid grid-cols-1 place-items-center items-start gap-14 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
               {FeaturesCardsData.map(item => (
                  <FeaturesCard
                     className={`h-full md:w-full md:max-w-full md:last:col-span-2 lg:max-w-[327px] lg:last:col-span-1 lg:last:w-auto lg:last:max-w-[327px]`}
                     key={item.id}
