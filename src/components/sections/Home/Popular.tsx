   ...
   return (
      <section className={`mb-10 py-20 md:mb-40`}>
         <div className="container">
            <Title
               className={`mx-auto mb-[15px] w-full max-w-[500px] text-center text-[48px] font-black leading-[50px]`}
               tag={'h3'}
               text={t('title')}
            />
            <Text
               className={`mx-auto mb-[30px] max-w-[780px] text-center`}
               text={t('text')}
            />
            <div className={`grid grid-cols-3 justify-between gap-8`}>
               {popularCardsImages.map(item => (
                  ...
                  >
                     ...
                     />
                     <span
                        className={`text-[30px] font-bold leading-[70px] text-gray-2`}
                     >
                        {t(`cardsTitles.${item.id}`)}
                     </span>
                     <Text
                        className={``}
                        text={t(`cardsTexts.${item.id}`)}
                     />
                  </div>
               ))}
            </div>
