...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-[#38394E] w-full md:max-w-[40%] p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 dark:bg-slate-950',
  {
    variants: {
      side: {
        ...
        right: 'inset-y-0 right-0 h-full data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-[948px]'
      }
    },
    defaultVariants: {
      side: 'right'
...
const SheetTitle = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Title>, React.ComponentPropsWithoutRef<typeof SheetPrimitive.Title>>(
  ({ className, ...props }, ref) => (
    <SheetPrimitive.Title ref={ref} className={cn('text-[25px] font-medium leading-[24px] text-[#C7C7C7]', className)} {...props} />
  )
)
